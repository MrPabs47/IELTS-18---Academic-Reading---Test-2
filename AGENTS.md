...
Full-screen verification additions:

- Confirm Test mode attempts full screen at start.
- Confirm leaving full screen pauses the test.
- Confirm **Return to full screen** resumes the test.
- Confirm Study mode does not enforce full screen.
- Confirm results show full-screen exits, focus losses, and total time.
- Confirm the timer does not duplicate or continue running behind the lock overlay.
- Confirm submit confirmation still works.
- Confirm `#fullscreenLockOverlay` is hidden by default.
- Confirm `#fullscreenLockPanel` is styled and readable.
