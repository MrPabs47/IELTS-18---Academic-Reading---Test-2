(function () {
  ...
  function prepareInstructionHosts() {
    ...
    if (s1) {
      wrapRange(s1, directChildByText(s1, "p", "SECTION 1 Questions"), s1.querySelector('.question-block[data-q="1"]'), "study-instruction-s1-cafes");
      wrapRange(s1, directChildByText(s1, "p.question-group-heading", "Frog Valley development"), s1.querySelector('.question-block[data-q="6"]'), "study-instruction-s1-frog");
    }
    if (s2) {
      wrapRange(s2, directChildByText(s2, "p", "SECTION 2 Questions"), s2.querySelector('.question-block[data-q="15"]'), "study-instruction-s2-institute");
      wrapRange(s2, directChildByText(s2, "p.question-group-heading", "Percil Memorial Training Scholarships"), s2.querySelector('.question-block[data-q="25"]'), "study-instruction-s2-scholarships");
    }
    if (s3) {
      wrapRange(s3, directChildByText(s3, "p", "SECTION 3 Questions"), s3.querySelector('.summary-completion-box'), "study-instruction-s3-summary");
      wrapRange(s3, directChildByText(s3, "p", "Questions 31–36"), s3.querySelector('.question-block[data-q="31"]'), "study-instruction-s3-people");
      wrapRange(s3, directChildByText(s3, "p", "Questions 37–40"), s3.querySelector('.question-block[data-q="37"]'), "study-instruction-s3-mc");
    }
  }
  ...
  function prepareTextRoots() {
    ...
    wrapBeforeDivider(s1, "text-s1-cafes");
    if (s1) {
      var frog = s1.querySelector(":scope > .passage-divider");
      if (frog) { frog.id = "text-s1-frog"; frog.classList.add("reading-text"); }
    }
    wrapBeforeDivider(s2, "text-s2-institute");
    if (s2) {
      var scholarships = s2.querySelector(":scope > .passage-divider");
      if (scholarships) { scholarships.id = "text-s2-scholarships"; scholarships.classList.add("reading-text"); }
    }
    if (s3) { s3.id = "text-s3-rewilding"; s3.classList.add("reading-text"); }
  }

  function prepareInlineFeedbackHosts() {
    for (var question = 28; question <= 30; question += 1) {
  ...
  function installTest4Styles() {
    if (document.getElementById("gt-test4-study-styles")) return;
    var style = document.createElement("style");
    style.id = "gt-test4-study-styles";
    style.textContent = [
      ".gt-test4-header-left{min-width:0;flex:1 1 auto;overflow:hidden;}",
      ".gt-test4-header-left .test-title,.gt-test4-header-left #candidateNameDisplay{font-size:.95rem;color:var(--text-soft);min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}",
      ".gt-test4-header-left .test-title{flex:0 1 auto;}",
  ...
  function prepareChrome() {
    ...
    if (topLeft) topLeft.classList.add("gt-test4-header-left");
    if (topRight) topRight.classList.add("gt-test4-header-right");

    var title = topLeft && topLeft.querySelector(".test-title");
    if (title) title.textContent = "IELTS 19 General Training Reading Test 4";
    document.title = "IELTS 19 General Training Reading Test 4 | IELTS Pabs";

    if (topLeft && !document.getElementById("candidateNameDisplay")) {
  ...
  function lockSubmittedTest() {
    ...
    document.querySelectorAll(".gt-section1-clear,.clear-drop-btn").forEach(function (button) {
  ...
  function wrapReadingFeatureShellLocalisation() {
    if (!window.ReadingFeatureShell || window.ReadingFeatureShell.__gtTest4Localised) return;
    ...
    window.ReadingFeatureShell.__gtTest4Localised = true;
  ...
  function installSubmittedInteractionGuard() {
    ["click", "keydown", "dragstart", "drop"].forEach(function (type) {
      document.addEventListener(type, function (event) {
        if (mode !== "test" || !testSubmitted) return;
        var target = event.target && event.target.closest ? event.target.closest(".passage-match-source,.drop-zone,.gt-section1-clear,.clear-drop-btn") : null;
        if (!target) return;
  ...
  function initStudyMode() {
    ...
    installSubmittedInteractionGuard();
    patchPageFunctions();
    renderSectionHeader();
    syncModeUi();

    var feedback = window.IELTS19GTTest4StudyFeedback;
    if (!feedback || !window.ReadingFeatureShell) {
      console.warn("IELTS 19 GT Test 4 Study Mode assets could not load.");
      return;
    }
    ...
    window.readingFeatureShellConfig = {
      version: 1,
      test: {
        id: "cambridge-19-general-training-reading-test-4",
        title: "IELTS 19 General Training Reading Test 4",
        totalQuestions: 40,
        partLabel: "Section",
    ...
    if (!shellInit || shellInit.ok !== true) console.warn("IELTS 19 GT Test 4 Study Mode could not initialise.", shellInit && shellInit.error);
