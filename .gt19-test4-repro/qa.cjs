...
(async () => {
  ...
  try {
    ...
    assert.ok(
      ...
      'Submitted score feedback must work even for below-Band-3 results.'
    );
    await page.getByRole('button', { name: /close score feedback/i }).click();

    const firstInfo = page.locator('.reading-shell-study-icon-button:visible').first();
